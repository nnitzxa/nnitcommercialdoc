...
CREATE TABLE IF NOT EXISTS `mydb`.`Member` (
  ...
  `STATUS` VARCHAR(255) NOT NULL COMMENT '用户的状态\n新注册的用户都是“激活”状态',
  `IS_ONLINE` VARCHAR(255) NOT NULL COMMENT '是否在线状态',
  `GENDER` VARCHAR(255) NULL COMMENT '性别',
  `PIC` VARCHAR(120) NULL COMMENT '头像（一个PIC地址。可能是一个微软云）',
  `EMAIL_ADDR` VARCHAR(25) NULL COMMENT '电子邮件地址',
  `CELL_PHONE` VARCHAR(15) NULL COMMENT '手机号码（这个必须让用户注册）',
  `TYPE` VARCHAR(45) NULL COMMENT '会员的类型',
  PRIMARY KEY (`ID`))
ENGINE = InnoDB
COMMENT = '会员基本信息表';
...
CREATE TABLE IF NOT EXISTS `mydb`.`IntegralTransaction` (
  ...
  `TRX_TYPE_ID` VARCHAR(255) NOT NULL COMMENT '交易类型：购买、撤回 ',
  `TRX_DATETIME` DATETIME NOT NULL COMMENT '交易日期时间',
  `MEMBER_ID` VARCHAR(255) NOT NULL COMMENT '交易的用户',
  `STATUS_ID` VARCHAR(255) NULL COMMENT '记录成功还是失败',
...
CREATE TABLE IF NOT EXISTS `mydb`.`Couponbackage` (
  `ID` MEDIUMTEXT GENERATED ALWAYS AS () VIRTUAL,
  `CODE` VARCHAR(45) NULL COMMENT '优惠券的编码. 这个编码需要在APP端生成条码或者二维码（建议用二维码）\n这个编码必须和“discount_user”表中的code一样',
  `COUPON_TYPE` VARCHAR(255) NULL COMMENT '优惠券类型：\n团购券\n优惠券\n活动券\n礼品兑换券',
  `STATUS_ID` VARCHAR(255) NULL COMMENT '优惠券的状态\n未使用\n已使用\n过期',
...
CREATE TABLE IF NOT EXISTS `mydb`.`Order` (
  `ID` VARCHAR(255) NOT NULL COMMENT '订单流水编号（订单号）',
  `ORDER_TYPE_ID` VARCHAR(255) NULL COMMENT '订单类型',
  `ORDER_DATETIME` DATETIME NULL COMMENT '订单生成日期',
  `AMOUNT` DECIMAL NULL COMMENT '订单总价格 - 如果使用优惠券，订单价格可以为0',
  `MEMBER_ID` MEDIUMTEXT NOT NULL COMMENT '订单所属会员',
  `STATUS_ID` INT NULL COMMENT '订单状态',
  `IS_USED_DISCOUNT` TINYINT(1) NULL COMMENT '是否使用优惠券',
  `DISCOUNT_ID` VARCHAR(255) NULL COMMENT '使用的优惠券的ID（每笔交易只能使用一个优惠券。以后可以扩展，但是需要拆表）',
  PRIMARY KEY (`ID`),
...
CREATE TABLE IF NOT EXISTS `mydb`.`shop` (
  ...
  `TRUE_SCENE` VARCHAR(1000) NULL COMMENT '实景图（指向云存储的一个UIR。目前，一个商铺就提供一个实景图）',
  `TELEPHONE` VARCHAR(255) NOT NULL COMMENT '商铺的联系电话',
  `CONTACT` VARCHAR(255) NOT NULL COMMENT '店铺联系人（或者法人）',
  `CONTACT_TEL` VARCHAR(255) NULL COMMENT '联系人（法人）的电话',
  `INTRODUCTION` TEXT NOT NULL COMMENT '商户的简介',
  `CATEGORY_ID` VARCHAR(255) NOT NULL COMMENT '店铺对应的类别',
  `CREATE_TIME` DATETIME NOT NULL COMMENT '店铺创建时间（在系统里创建的时间，不是店铺开业时间）',
  `OPENING_TIME` DATETIME NULL COMMENT '店铺开业时间',
  `MEMBER_ID` VARCHAR(255) NOT NULL COMMENT '该店铺的对应这个系统的管理员',
  PRIMARY KEY (`ID`),
...
CREATE TABLE IF NOT EXISTS `mydb`.`evaluation` (
  ...
  `MEMBER_ID` VARCHAR(255) NOT NULL COMMENT '评论发出的人',
  `PIC` VARCHAR(255) NULL COMMENT '品论可以加图片。这里记录图片的URI',
  `SHOP_ID` VARCHAR(255) NOT NULL COMMENT '此条评论发给的商铺',
  PRIMARY KEY (`ID`),
...
CREATE TABLE IF NOT EXISTS `mydb`.`Groupon` (
  `ID` VARCHAR(255) NOT NULL,
  `SHOP_ID` VARCHAR(255) NOT NULL COMMENT '团购活动所属的商铺',
...
CREATE TABLE IF NOT EXISTS `mydb`.`Groupon_order` (
  `ID` VARCHAR(255) NOT NULL,
  `GROUPON_ID` VARCHAR(255) NOT NULL COMMENT '该团购交易对应的团购活动',
  `CREATE_TIME` DATETIME NOT NULL COMMENT '此订单产生的时间',
  `AMOUNT` DECIMAL(10,2) NOT NULL COMMENT '此次团购的总金额',
  `STATUS_ID` VARCHAR(255) NOT NULL COMMENT '团购订单状态（已提交，已付款，已使用，退款中，已退款，已作废）\n关联到TYPE_AND_STATUS表',
  `SETTLEMENT_TIME` DATETIME NULL COMMENT '付款时间',
  `ORDER_TYPE` VARCHAR(255) NULL COMMENT '交易类型',
  `MEMBER_ID` VARCHAR(255) NOT NULL COMMENT '团购购买者',
  `SHOP_ID` VARCHAR(255) NULL COMMENT '团购订单归属的商铺',
  PRIMARY KEY (`ID`),
...
CREATE TABLE IF NOT EXISTS `mydb`.`product` (
  ...
  `SHOP_ID` VARCHAR(255) NOT NULL COMMENT '外键，确定商品归属的商铺',
...
CREATE TABLE IF NOT EXISTS `mydb`.`favorit_shop` (
  `ID` VARCHAR(255) NULL COMMENT '唯一标识',
...
CREATE TABLE IF NOT EXISTS `mydb`.`order_product` (
  `ID` VARCHAR(255) NULL COMMENT '表记录编号',
  `PRODUCT_ID` VARCHAR(255) NOT NULL COMMENT '购买的商品编号',
  `ORDER_ID` VARCHAR(255) NOT NULL COMMENT '对应的交易记录',
...
CREATE TABLE IF NOT EXISTS `mydb`.`IntegralTransaction_product` (
  `ID` VARCHAR(255) NULL COMMENT '编号',
  `PRODUCT_ID` VARCHAR(255) NOT NULL COMMENT '交易的商品编号',
  `TRANSACTION_ID` INT NOT NULL COMMENT '对应的积分交易编号',
...
CREATE TABLE IF NOT EXISTS `mydb`.`discount` (
  ...
  `SHOP_ID` VARCHAR(255) NOT NULL COMMENT '优惠活动所属的商铺',
  PRIMARY KEY (`ID`),
...
CREATE TABLE IF NOT EXISTS `mydb`.`discount_user` (
  `ID` VARCHAR(255) NOT NULL,
  `DISCOUNT_ID` VARCHAR(255) NULL COMMENT '对用优惠的编号标识',
  `MEMBER_ID` VARCHAR(255) NULL COMMENT '可以使用该优惠的用户标识',
  `CODE` VARCHAR(45) NULL COMMENT '优惠产生的编码（不同的用户，即使是同一个优惠活动，这个编码还是不同的）\n采用的算法：\n用户ID、优惠ID、商户ID、优惠时间，这4个参数做一个算法',
  `STATUS_ID` VARCHAR(255) NULL COMMENT '该用户是否已经使用了该优惠\n已用、未用、过期',
  PRIMARY KEY (`ID`))
ENGINE = InnoDB
